  ...
  if (view.name === "search") {
    return (
      <div className="px-5 pt-8 pb-32">
        ...
        <h1 className="font-display font-extrabold text-3xl leading-tight mb-5 text-balance">
        ...
        {filteredCategories.length === 0 ? (
          <div className="border border-dashed border-line rounded-card px-6 py-10 text-center">
            ...
            <p className="text-muted text-sm mb-2 max-w-[30ch] mx-auto">
              Not one of our five categories, and not a subtype or dish name either.
            </p>
  ...
  if (view.name === "results") {
    ...
    return (
      <div className="px-5 pt-6 pb-32">
        ...
        <p className="font-mono text-[11px] tracking-[0.1em] uppercase text-faint mb-3">
          Ranked in Bangalore · {dishes.length} logged
        </p>
        <div className="flex flex-col gap-2">
          {dishes.map((d, i) => (
            <motion.button
              key={d.id}
              onClick={() => setView({ name: "profile", dish: d })}
              initial={{ opacity: 0, y: 8 }}
              animate={{ opacity: 1, y: 0 }}
              transition={{ duration: 0.2, delay: i * 0.04, ease: [0.16, 1, 0.3, 1] }}
              whileTap={{ scale: 0.98 }}
              className="flex items-center gap-3 bg-surface border border-line rounded-xl p-3 text-left hover:border-accent/50 transition-colors"
            >
              <span className="font-mono text-faint text-sm w-4 tabular">{i + 1}</span>
